...
example : ∀ (n:Nat), True := fun n => True.intro
#check ∀ (n:Nat), True

/-!
A proof of a forall proposition is really just a proof that
for any arbitrarily selected element of the quantified type, there
is proof of the predicate applied to that object.
-/
...
/-!
Statement that all dogs are blue in predicate logic
-/
#check ∀ (n : Nat), Nat
#check ∀ (d : Dog), Blue d
...
variable
  ...
  (Iris : Dog)                            -- Iris is one
  (Blue : Dog → Prop)                     -- The property of being blue
  (iris_is_blue: Blue Iris)   -- Proof all dogs are blue

example : ∃ (d : Dog), Blue d :=
Exists.intro Iris iris_is_blue

example : ∃ (d : Dog), Blue d := ⟨Iris, iris_is_blue⟩ -- same thing as above
...
-- | Exists.intro w pf => Exists.intro w pf.2 -- does same thing as below
| ⟨ w, pf_w ⟩ => Exists.intro w pf_w.2
